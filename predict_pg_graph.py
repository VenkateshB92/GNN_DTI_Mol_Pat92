...
def draw_graph(data, explanation, word, word1):
    ...
    node_values = np.array(node_mask)
    ...
    edge_norm = plt.Normalize(vmin=min(edge_values), vmax=max(edge_values))
    ...
    nx.set_node_attributes(g, {i: value for i, value in enumerate(node_values)}, 'value')
...
def predicting(model, device, loader):
    ...
    for data in loader:
        ...
        for epoch in range(30):
            target = data.y
            explainer.algorithm.train(epoch, model, data.x, data.edge_index, target=target, index=0)

        explanation = explainer(data.x, data.edge_index, target=data.y, index=0)
        ...
        try:
            ...
            print(f"Error in drawing graph {i}: {e}")
            continue

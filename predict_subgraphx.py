...
def draw_graph(data, explanation, word, word1):
    ...
    nx.set_node_attributes(g, node_mask_values, 'node_mask')

    edge_mask = explanation.edge_mask.numpy()
    edge_values = {i: value for i, value in enumerate(edge_mask)}
    for edge, weight in zip(g.edges(), edge_values.values()):
        g[edge[0]][edge[1]]['weight'] = weight

    labels = set_labels(data.x)

    node_values = np.array([data['value'] for _, data in g.nodes(data=True)])
    edge_values = np.array([g[u][v]['weight'] for u, v in g.edges()])
...
def predicting(model, device, loader):
    ...
    explainer = SubgraphX(model=model, ## Make suitable changes to these hyperparameters
                          num_hops=3,  
                          coef=10.0, 
                          high2low=True, 
                          num_child=12, 
